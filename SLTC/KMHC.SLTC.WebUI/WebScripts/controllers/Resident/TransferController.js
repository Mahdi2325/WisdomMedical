...
.controller("transferRecCtrl", ['$scope', 'dictionary', 'utility', 'resourceFactory', function ($scope, dictionary, utility, resourceFactory) {
    var transferRes = resourceFactory.getResource("transferRes");
    $scope.Data = {};
    // 當前住民
    ...
    $scope.listItem = function (resident) {
        ...
        transferRes.query({}, function (data) {
    ...
    $scope.deleteItem = function (item) {
        if (confirm("您确定要删除该住民的亲友访探记录吗?")) {
            transferRes.delete({ id: item.id }, function () {
    ...
    $scope.createItem = function (item) {
        ...
        transferRes.save($scope.currentItem, function (data) {
    ...
    $scope.updateItem = function (item) {
        transferRes.save(item, function (data) {
            $scope.currentItem = {};
        });
    };
